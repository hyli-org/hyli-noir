...
[dependencies]
noir_utils = { path = "${noirUtilsRelativePath}" }
...
// Blob size for DSC circuits: comm_out (32 bytes) + certificate_registry_root (32 bytes) = 64 bytes
const DSC_BLOB_SIZE = 64
...
${unconstrained ? "unconstrained " : ""}fn main(
    ...
    let mut blob: BoundedVec<u8, ${DSC_BLOB_SIZE}> = BoundedVec::new();
    ...
    assert(hyli.blob_capacity == ${DSC_BLOB_SIZE});
    assert(hyli.blob_len == ${DSC_BLOB_SIZE});
...
// Blob size for ID Data circuits: comm_in (32 bytes) + comm_out (32 bytes) = 64 bytes
const ID_DATA_BLOB_SIZE = 64
...
${unconstrained ? "unconstrained " : ""}fn main(
    hyli: HyliOutput<${ID_DATA_BLOB_SIZE}>,
    comm_in: Field,
    salt_in: Field,
    salt_out: Field,
    dg1: DG1Data,
    ...
    let mut blob: BoundedVec<u8, ${ID_DATA_BLOB_SIZE}> = BoundedVec::new();
    ...
    assert(hyli.initial_state == [0; 4]);
    assert(hyli.next_state == [0; 4]);
    assert(hyli.blob_capacity == ${ID_DATA_BLOB_SIZE});
    assert(hyli.blob_len == ${ID_DATA_BLOB_SIZE});
    assert(hyli.blob_number == 1);
    assert(hyli.tx_blob_count >= 1);
}
`
...
// Blob size for Data Integrity Check circuits: comm_in (32 bytes) + comm_out (32 bytes) = 64 bytes
const DATA_INTEGRITY_BLOB_SIZE = 64
...
${unconstrained ? "unconstrained " : ""}fn main(
    hyli: HyliOutput<${DATA_INTEGRITY_BLOB_SIZE}>,
    comm_in: Field,
    salt_in: Field,
    ...
    // Build blob: comm_in (32 bytes) + comm_out (32 bytes)
    let mut blob: BoundedVec<u8, ${DATA_INTEGRITY_BLOB_SIZE}> = BoundedVec::new();
    blob.extend_from_array(comm_in.to_be_bytes::<32>());
    blob.extend_from_array(comm_out.to_be_bytes::<32>());
    ...
    assert(hyli.initial_state == [0; 4]);
    assert(hyli.next_state == [0; 4]);
    assert(hyli.blob_capacity == ${DATA_INTEGRITY_BLOB_SIZE});
    assert(hyli.blob_len == ${DATA_INTEGRITY_BLOB_SIZE});
    assert(hyli.blob_number == 1);
    assert(hyli.tx_blob_count >= 1);
}
`

// Blob size for FaceMatch circuits: param_commitment (32) + nullifier_type (32) + nullifier (32) = 96 bytes
const FACEMATCH_BLOB_SIZE = 96
...
${unconstrained ? "unconstrained " : ""}fn main(
    hyli: HyliOutput<${FACEMATCH_BLOB_SIZE}>,
    comm_in: Field,
    current_date: u64,
    salted_private_nullifier: SaltedValue<Field>,
    ...
    let mut blob: BoundedVec<u8, ${FACEMATCH_BLOB_SIZE}> = BoundedVec::new();
    ...
    assert(hyli.initial_state == [0; 4]);
    assert(hyli.next_state == [0; 4]);
    assert(hyli.blob_capacity == ${FACEMATCH_BLOB_SIZE});
    assert(hyli.blob_len == ${FACEMATCH_BLOB_SIZE});
    assert(hyli.blob_number == 1);
    assert(hyli.tx_blob_count >= 1);
}
`

// Blob size for Outer circuits:
...
// param_commitments (32 * n) = 200 + 32*n bytes
// For simplicity, we use a fixed large size to accommodate up to 10 disclosure proofs
const OUTER_BLOB_SIZE = 520 // 200 + 32*10 = 520
...
${unconstrained ? "unconstrained " : ""}fn main(
    hyli: HyliOutput<${OUTER_BLOB_SIZE}>,
    ...
    let mut blob: BoundedVec<u8, ${OUTER_BLOB_SIZE}> = BoundedVec::new();
    blob.extend_from_array(certificate_registry_root.to_be_bytes::<32>());
    ...
    assert(hyli.initial_state == [0; 4]);
    assert(hyli.next_state == [0; 4]);
    assert(hyli.blob_capacity == ${OUTER_BLOB_SIZE});
    assert(hyli.blob_number == 1);
    assert(hyli.tx_blob_count >= 1);
}
`
...
function generateDscEcdsaCircuit(
  ...
  // Path from zkpassport/src/noir/bin/sig-check/dsc/tbs_X/ecdsa/family/curve/hash/ to noir-utils/
  // 10 levels in folder + 1 to get out of zkpassport = 11 levels up
  const noirUtilsPath = "../../../../../../../../../../../noir-utils"
...
function generateDscRsaCircuit(
  ...
  // Path from zkpassport/src/noir/bin/sig-check/dsc/tbs_X/rsa/type/bits/hash/ to noir-utils/
  // 10 levels in folder + 1 to get out of zkpassport = 11 levels up
  const noirUtilsPath = "../../../../../../../../../../../noir-utils"
  const nargoFile = NARGO_TEMPLATE(name, [
...
function generateIdDataEcdsaCircuit(
  ...
  // Path from zkpassport/src/noir/bin/sig-check/id-data/tbs_X/ecdsa/family/curve/hash/ to noir-utils/
  // 10 levels in folder + 1 to get out of zkpassport = 11 levels up
  const noirUtilsPath = "../../../../../../../../../../../noir-utils"
  const nargoFile = NARGO_TEMPLATE(name, [
    { name: "sig_check_ecdsa", path: "../../../../../../../../lib/sig-check/ecdsa" },
...
function generateIdDataRsaCircuit(
  ...
  // Path from zkpassport/src/noir/bin/sig-check/id-data/tbs_X/rsa/type/bits/hash/ to noir-utils/
  // 10 levels in folder + 1 to get out of zkpassport = 11 levels up
  const noirUtilsPath = "../../../../../../../../../../../noir-utils"
  const nargoFile = NARGO_TEMPLATE(name, [
    { name: "sig_check_rsa", path: "../../../../../../../../lib/sig-check/rsa" },
...
function generateDataIntegrityCheckCircuit(
  ...
  // Path from zkpassport/src/noir/bin/data-check/integrity/sa_X/dg_Y/ to noir-utils/
  // 7 levels in folder + 1 to get out of zkpassport = 8 levels up
  const noirUtilsPath = "../../../../../../../../noir-utils"
  const nargoFile = NARGO_TEMPLATE(name, [
...
function generateFaceMatchAndroidCircuit(
  ...
  // Path needs to go from the circuit folder up to zkpassport level, then up one more to get to noir-utils
  // Base: src/noir/bin/facematch/android/rk_X/ik_count_Y/(ik paths)/standard|evm = 7 + Y + 1 levels
  // Plus one more to get out of zkpassport
  const noirUtilsPath = `${relativePath}../../noir-utils`
  const nargoFile = NARGO_TEMPLATE(name, [
...
function generateOuterCircuit(disclosure_proofs_count: number, unconstrained: boolean = false) {
  ...
  // Path from zkpassport/src/noir/bin/main/outer/count_X/ to noir-utils/
  // 6 levels in folder + 1 to get out of zkpassport = 7 levels up
  const noirUtilsPath = "../../../../../../../noir-utils"
  const nargoFile = NARGO_TEMPLATE(name, [
